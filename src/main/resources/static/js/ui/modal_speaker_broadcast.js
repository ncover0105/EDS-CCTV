...
const BroadcastModal = {
    ...
    // ✅ 수정: 재난 선택 시 TTS 숨김 + (TTS는 type=1 이고 재난 미선택일 때만)
    applyBroadcastTypeUI() {
        ...
        const type = typeEl?.value ?? "1";          // 1=TTS, 2=저장메시지, 3=기타
        const disaster = disEl?.value ?? "";        // 재난 선택 값
        const hasDisaster = !!String(disaster).trim();

        // TTS 입력은 "방송종류=TTS(1)" + "재난 미선택"일 때만 표시
        const shouldShowTts = (type === "1") && !hasDisaster;

        if (wrap) wrap.classList.toggle("d-none", !shouldShowTts);

        // 숨겨질 때 값 초기화 + 비활성화(잘못 전송 방지)
        if (tts) {
